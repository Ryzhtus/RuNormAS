...
class RuNormASReaderForSequenceTagging():
    ...
    def find_entity_by_spans(self, token, entities_spans):
            for idx in range(len(entities_spans)):
                if entities_spans[idx][0] == token.start or entities_spans[idx][1] == token.stop:
                    return True
...
def collect_sentences_for_sequence_tagging():
    ...
    for filename in tqdm.tqdm(train, total=len(train)):
        ...
        document, document_entities, document_normalization, document_entities_spans = reader.read(text_filename,
                                                                                                   annotation_filename,
                                                                                                   normalization_filename)
        sentences, sentences_endings, sentences_entities_ids = reader.parse_entities(document, document_normalization,
